...
// PutLock ads a key/lock to this transaction.
...
func (txn *MvccTxn) GetValue(key []byte) ([]byte, error) {
	...
	pointer:=iter.Item()
	findEncodeKey:=pointer.Key()
	findKey:=DecodeUserKey(findEncodeKey)
	if !bytes.Equal(findKey,key){
		return nil,nil
	}
...
func (txn *MvccTxn) CurrentWrite(key []byte) (*Write, uint64, error) {
	...
	for iter.Seek(EncodeKey(key,^uint64(0)));iter.Valid();iter.Next(){
		...
		if write.StartTS==txn.StartTS{
			return write,decodeTimestamp(findEncodeKey),nil
		}
	}
	return nil,0,nil
}
